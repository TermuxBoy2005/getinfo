#! /usr/bin/env python3
# Author by @termuxboy2005
# Support me with follow my facebook page https://www.facebook.com/deni6660
# Disclaimer: please dont re-edit or recode the original source code !
# Last update: 12/08/2021 - version 1.0
...
logo = f"""{b}
      ...
      <KKKKKKKKkr,                       ,rSKKKKKKKK<
       -"v]qj;-                             -;jq]v"-
                     {w}[ G.E.T.I.N.F.O ]{b}
                {d}Simple Information Toolkit{w}
                 {d}Author by {w}{r}@termuxboy2005{w}"""

def menu():
    os.system("clear")
    print(logo)
    print(f"""
              {W}\033[2;30m Choose number which you want {w}
        ...
        {w}{b}  00{r} Exit         {d} Get Out From Here
        """)
    mainmenu()
...
def mainmenu():
    while True:
        try:
            ...
            if int(len(cmd)) < 6:
                if cmd in ("0","00"): exit(r+space+"* Exiting...!!!"+w)
        ...
        if "mbasic_logout_button" in str(req):
            if "Apa yang Anda pikirkan sekarang" in str(req):
                ...
                try:
                    ...
                    x = parser(requests.get(mbasic.format("/deni6660"),cookies=cookies).content,"html.parser").find("a",string="Ikuti")["href"]
...
if __name__ == "__main__":
    ...
    if len(arg) == 1: menu()
        ...
        if arg[1] in ("update"):
            if which("termux-setup-storage"): path = "$PREFIX/bin/getinfo"
            else:
                if os.path.isdir("/usr/local/bin/"): path = "/usr/local/bin/getinfo"
                else: path = "/usr/bin/getinfo"
            os.system(f"wget https://raw.githubusercontent.com/majidtdeni666/getinfo/main/get.py -O {path} && chmod +x {path}")
